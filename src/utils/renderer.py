...
class PygameFrontend:
    ...
    def draw_rays_transparent(self):
        ...
        for vec in self.rays:
            end_px = (
                int(agent_px[0] + vec[1] * self.CELL_SIZE),
                int(agent_px[1] + vec[0] * self.CELL_SIZE),
            )
            pygame.draw.line(ray_surf, (255, 0, 0, self.alpha), agent_px, end_px, 2)
